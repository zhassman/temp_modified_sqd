...
def postselect_by_hamming_right_and_left(
    ...
    *,
    ...
    if return_true_probs:
        ...
        nums = [int(''.join(str(int(x)) for x in bitstring), 2) for bitstring in bitstrings_post]
        print("Using true probabilities from matchgate simulator.")

        if pool_size is not None:
            filename = f'{trajectory_count}_trajectories/bitstring_to_probability/{pool_size}_bitstrings_{trajectory_count}_trajectories_bitstring_to_probability.npy'
            print(f'Collecting probabilities from {filename}')
            prob_map = np.load(filename, allow_pickle=True).item()
            true_probs = [prob_map[i] for i in nums]
        else:
            print("Obtaining probabilities with raw estimate.")
            true_probs = raw_estimate_lucj(circuit=circuit,
                                    outcome_states=nums,
                                    trajectory_count=trajectory_count)

        probs_post = true_probs / np.sum(true_probs)
...
def subsample(
    ...
    print("Num bitstrings to subsample from:", num_bitstrings)
    ...
    if samples_per_batch >= num_bitstrings:
        ...
        if choose_best:
            print(f"You have set --samples_per_batch = {samples_per_batch}, but there are only {num_bitstrings} valid bitstrings.")
    ...
    for _ in range(num_batches):
        if randomly_sample:
            ...
            if choose_best:
                print("choose_best set to True. Choosing most likely bitstrings.")
                indices = np.argsort(probabilities)[-samples_per_batch:]
